...
<div class="row" style="margin-top: 78px;margin-left: 1px;margin-right: 1px;">
    <div class="col-sm-12">
        <div class="card-box table-responsive">
            <div class="col-sm-2">
                <button onclick="add_groups()" class="btn btn-custom waves-effect w-md waves-light btn-xs"
                        style="margin-top: -3px;padding: 4px;border-radius: 5px;font-size:15px;margin-left: 0px;"><i class="fa fa-plus"></i>&nbsp;&nbsp;添加用户组
                </button>
            </div>
        </div>
    </div>
    ...
    <script type="text/javascript">
        function loadData(groups) {
            ...
            $("#datatable").dataTable({
                ...
                "ajax": { // 请求地址
                    "url": "/ci/env/permissions/groups/data?t=" + new Date().getTime() + "&groups=" + groups,
                    "type": 'post'
                },
                "columns": [ // 数据映射
                    {"data": "groupsName"},
                    ...
                    {"data": "description"},
                    {
                        ...
                        return ' <button onclick="add_groups('+data+')" class="btn btn-custom waves-effect  waves-light btn-xs"' +
        ...
        function add_groups(id) {
            if(!id){
                id=0;
            }
            make_page("/ci/env/permissions/groups/add", "/应用管理/权限管理/添加组", "?groupsId="+id);
        }

        function delete_groups(id, name) {
                parent.layer.confirm("点击同意后,将删除组"+name, {
                    btn: ['同意', '不同意'], //按钮
                    shade: false//不显示遮罩
                }, function () {
                    var url = "/ci/env/permissions/groups/delete/"+id;
                    var result = post({}, url);
